...
async function onStartup(): Promise<void> {
  ...
  try {
    // A pane in Zotero's Settings window, beside the other plugins. Must run
    // in THIS hook: BatchOpenPlugin has no bootstrap-invoked startup().
    const base = typeof rootURI !== "undefined" ? rootURI : "";
    (
      Zotero as unknown as {
        PreferencePanes: { register(o: object): void };
      }
    ).PreferencePanes.register({
      pluginID: addon.data.config.addonID,
      src: `${base}content/preferences.xhtml`,
      label: "Batch Open",
    });
  } catch (error) {
    console.log(`[Batch Open] Settings pane registration failed: ${error}`);
  }
